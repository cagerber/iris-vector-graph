...
| `module.xml` / `module-core.xml` / `module-vector.xml` / `module-mcp.xml` | **IPM completeness** — all 44 `iris_src/src/**/*.cls` resources; version `2.5.0-trifour.6` (wheel `2.5.0+trifour.6`) |
...
| `IVG.Exec` (renamed from `User.Exec`, trifour.5) | **IRIS Health ZPM fix** — `User` package name conflicts with `%IPM`→`USER` mapping during Reload |
| `IVG.PageRankEmbedded` (renamed from root `PageRankEmbedded`, trifour.6) | **IRIS Health ZPM fix** — root-level class (empty package) triggers `Config.MapPackages` `<SUBSCRIPT>` on `^SYS("CONFIG","IRIS","MapPackages",ns,"")` during full-module Reload |
