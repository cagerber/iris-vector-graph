...
> **This document is outdated (last updated at v1.47.0, current release is v1.84.0).**
...
> `ivf_*` (with `build_batch_size`), `ivf_insert`/`ivf_delete`, `KHopCount`/`KHop2Count` fast paths,
> `create_node(graph=)`, `get_labels`, `get_relationship_types`, the full introspection API, and
> **`engine.index(name)` → `IndexHandle`** — unified entry point for all index types (v1.84.0).
