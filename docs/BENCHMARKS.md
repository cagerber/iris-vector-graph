...
> **Container note**: This benchmark ran against Community IRIS _without_ `Graph.KG.Traversal` ObjectScript classes deployed. Multi-hop BFS uses the SQL fallback path (new in v1.94.0). With full ObjectScript deployment, 2–3 hop BFS is **5–10× faster**.
...
**When does EmbeddedConnection apply?** Any time IVG runs _inside_ IRIS:
